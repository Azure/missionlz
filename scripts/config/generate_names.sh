...
#
# Given an enclave name, generate MLZ resource names
...
mlz_sa_name_full="mlztfsa${mlz_enclave_name}${mlz_sub_id}"
mlz_kv_name_full="mlzkv${mlz_enclave_name}${mlz_sub_id}"

# Name MLZ config resources
export mlz_rg_name="rg-${mlz_prefix}-${mlz_enclave_name}"
export mlz_sp_name="${mlz_sp_name_full}"
export mlz_sp_kv_name="${mlz_sa_name_full}-clientid"
export mlz_sp_kv_password="${mlz_sa_name_full}-pwd"
...
if [[ $tf_name_raw != "notset" ]]; then
  # remove hyphens for resource naming restrictions
  # in the future, do more cleansing
  tf_sub_id="${tf_sub_id_raw//-}"
  tf_name="${tf_name_raw//-}"

  # TF naming patterns
  tf_prefix="tf-${tf_name}"
  tf_sa_name_full="tfsa${tf_name}${tf_sub_id}"
