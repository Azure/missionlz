...
- [ ] T001 [P] Create the `tests/` and `tests/artifacts/` directory structure at the repository root per [plan.md](plan.md) Project Structure.
- [ ] T002 [P] Add generated coverage/test outputs (`coverage.xml`, `testResults.xml`) to the repo-root `.gitignore` so ephemeral measurement artifacts are never committed (only `tests/coverage-baseline.json` is version-controlled).
