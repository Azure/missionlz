...
resource logAnalyticsWorkspace 'Microsoft.OperationalInsights/workspaces@2021-06-01' = {
  name: replace(replace(names.logAnalyticsWorkspace, tokens.purpose, 'avdi'), '${delimiter}${stampIndex}', '')
  location: location
...
module privateLinkScope_logAnalyticsWorkspace '../common/private-link-scope.bicep' = {
...
resource dataCollectionRule 'Microsoft.Insights/dataCollectionRules@2022-06-01' = if (enableAvdInsights) {
  name: 'microsoft-avdi-${replace(replace(names.dataCollectionRule, '${delimiter}${tokens.purpose}', ''), '${delimiter}${stampIndex}', '')}' // The name must start with 'microsoft-avdi-' for proper integration with AVD Insights
  location: location
...
resource dataCollectionEndpoint 'Microsoft.Insights/dataCollectionEndpoints@2021-04-01' = if (enableAvdInsights) {
  name: replace(replace(names.dataCollectionEndpoint, tokens.purpose, 'avdi'), '${delimiter}${stampIndex}', '')
  location: location
...
module privateLinkScope_dataCollectionEndpoint '../common/private-link-scope.bicep' = if (enableAvdInsights) {
