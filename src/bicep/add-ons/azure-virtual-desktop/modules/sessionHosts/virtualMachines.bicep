...
var fslogixExclusionsCloudCache = contains(fslogixContainerType, 'CloudCache')
  ...
  : ''
var fslogixExclusionsOfficeContainers = contains(fslogixContainerType, 'Office')
  ? ';"${fslogixOfficeShare}";"${fslogixOfficeShare}.lock";"${fslogixOfficeShare}.meta";"${fslogixOfficeShare}.metadata"'
  : ''
      ...
      availabilitySet: availability == 'AvailabilitySets'
        ? {
            id: resourceId(
              ...
              '${availabilitySetNamePrefix}-${padLeft((i + sessionHostIndex) / 200, 2, '0')}'
            )
          }
        : null
      hardwareProfile: {
...
resource extension_AzureMonitorWindowsAgent 'Microsoft.Compute/virtualMachines/extensions@2023-03-01' = [
  for i in range(0, sessionHostCount): if (monitoring) {
    parent: virtualMachine[i]
...
module runCommand '../common/runCommand.bicep' = [
  for i in range(0, sessionHostCount): {
...
resource extension_DesiredStateConfiguration 'Microsoft.Compute/virtualMachines/extensions@2021-03-01' = [
  for i in range(0, sessionHostCount): {
    parent: virtualMachine[i]
...
resource extension_JsonADDomainExtension 'Microsoft.Compute/virtualMachines/extensions@2021-03-01' = [
  for i in range(0, sessionHostCount): if (contains(activeDirectorySolution, 'DomainServices')) {
    parent: virtualMachine[i]
